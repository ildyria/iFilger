...
if iFilgerBuffConfig then 
	C["cooldown"] = iFilgerBuffConfig["Filger_Cooldown"]
end
...
local EXPIRING_FORMAT = I.RGBToHex(1, 0, 0)..'%.1f|r' --format for timers that are soon to expire
local SECONDS_FORMAT = I.RGBToHex(1, 1, 0)..'%d|r' --format for timers that have seconds remaining
local MINUTES_FORMAT = I.RGBToHex(1, 1, 1)..'%dm|r' --format for timers that have minutes remaining
local HOURS_FORMAT = I.RGBToHex(0.4, 1, 1)..'%dh|r' --format for timers that have hours remaining
local DAYS_FORMAT = I.RGBToHex(0.4, 0.4, 1)..'%dh|r' --format for timers that have days remaining
...
local function getTimeText(s)
	...
	if s < MINUTEISH then
		local seconds = tonumber(I.Round(s))
...
local function Timer_OnSizeChanged(self, width, height)
	local fontScale = I.Round(width) / ICON_SIZE
...
local function Timer_OnUpdate(self, elapsed)
	if self.nextUpdate > 0 then
		...
		if tonumber(I.Round(remain)) > 0 then
